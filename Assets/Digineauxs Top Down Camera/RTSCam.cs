...
public class RTSCam : NetworkBehaviour
{
  ...
  */
    ...
    void Start()
    {
        if (!IsOwner) return; // sadece kendi oyuncunun kamerasýný baþlat
    ...
    {
        if (!IsOwner) return; // sadece kendi oyuncunun inputlarýný uygula
        InputHandler();
